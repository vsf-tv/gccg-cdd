...
export class DefaultApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async setConfiguration(setConfigurationRequestContent: SetConfigurationRequestContent, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'setConfigurationRequestContent' is not null or undefined
        if (setConfigurationRequestContent === null || setConfigurationRequestContent === undefined) {
            throw new RequiredError("DefaultApi", "setConfiguration", "setConfigurationRequestContent");
        }
        ...
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(setConfigurationRequestContent, "SetConfigurationRequestContent", ""),
            contentType
        );
...
export class DefaultApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to setConfiguration
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async setConfigurationWithHttpInfo(response: ResponseContext): Promise<HttpInfo<void >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
